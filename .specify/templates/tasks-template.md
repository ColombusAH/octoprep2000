...
**Tests**: Include tests for backend routes, WebSocket/auth behavior, agent payload
contracts, scoring, and fallback paths when affected. Pure UI polish may use a documented
manual verification task instead of automated tests if the feature spec allows it.
...
- [ ] T010 Validate Constitution Check items from plan.md are represented in tasks
- [ ] T011 [P] Add or update shared payload types for changed WebSocket/agent contracts
...
- [ ] T014 [P] [US1] Auth/session isolation test for [route/socket] in tests/integration/test_[name].py
- [ ] T015 [P] [US1] Fallback/replay test for [provider/failure mode] in tests/integration/test_[name].py
...
- [ ] TXXX Verify `/health`, WebSocket reconnect, rate limit, `DEMO_MODE=replay`, and fallback behavior when touched
- [ ] TXXX Verify report UI on desktop and mobile when touched
